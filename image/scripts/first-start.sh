...
; do
    ...
    grep -rlZ -F "$placeholder" /etc /opt --exclude='*.db' 2>/dev/null | xargs -0 -r sed -i "s#${placeholder}#${real}#g"
done

echo "*** first start: rewiring build-time placeholder hostname/domain -> ${HOSTNAME_FQDN}/${MAIL_DOMAIN}"
grep -rlZ -F "$BUILD_HOSTNAME" /etc /opt --exclude='*.db' 2>/dev/null | xargs -0 -r sed -i "s#${BUILD_HOSTNAME}#${HOSTNAME_FQDN}#g"
grep -rlZ -F "$BUILD_DOMAIN" /etc /opt --exclude='*.db' 2>/dev/null | xargs -0 -r sed -i "s#${BUILD_DOMAIN}#${MAIL_DOMAIN}#g"
