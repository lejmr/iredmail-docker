...
| 21 | I move from the old `lejmr/iredmail:mysql-1.3*` image to this one without losing anything | on the old container: `mysqldump --all-databases` + tar of `/var/vmail`; on a fresh new server: `admin import-legacy dump.sql vmail.tar` | exit 0; every old domain in `admin domain list`; every old user logs in over IMAP with the old password and sees the old messages; quotas unchanged; `admin dkim <domain>` prints a record (keys are regenerated - DNS must be updated, and the command says so) | machine (fixture produced from the real old image) |
